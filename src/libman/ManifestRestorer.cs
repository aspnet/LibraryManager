        ...
        public static async Task RestoreManifestAsync(Manifest manifest, ILogger logger, CancellationToken cancelToken)
        {
            ...
            if (failures.Any())
            {
                ...
                foreach (ILibraryOperationResult f in failures)
                {
                    ...
                    {
                        librarySpecificErrors.AppendLine(string.Format(Resources.FailedToRestoreLibraryMessage, f.InstallationState.ToConsoleDisplayString(manifest.DefaultProvider, manifest.DefaultDestination)));
